...
<article class="relative">
    ...
    <x-ui.tabs
        ...
        <x-ui.tabs.list-wrapper class="w-full flex justify-between items-center h-12 px-3.5 bg-muted/50 relative">
            <x-ui.tabs.list class="bg-background shadow b b-border/50 p-0.5 rounded-md flex items-center">
                <x-ui.tabs.trigger id="previewbox"
                    class="px-3 py-1 flex gap-2 items-center text-sm fx-active:bg-muted/60 border border-transparent fx-active:border-border-strong/40 rounded-[5px] ease-linear duration-200 font-300 text-foreground fx-active:text-title-foreground ">
            ...
            <span class="mx-2.5 flex relative w-px bg-surface-elevated h-6" aria-hidden="true"></span>
            <div class="flex items-center flex-1">
                <span
                    class="px-3 py-1 bg-background shadow border border-border/50 flex gap-2 items-center text-sm rounded-[5px] ease-linear duration-200 font-300 text-foreground">
                    {{ $title }}
                </span>
            </div>
            <div class="flex items-center min-w-max text-foreground">
                <div data-box-resize-box
                    class="bg-background shadow border border-border/50 p-0.5 rounded-md flex items-center data-[position=fixed]:box-resize-controls">
                    <div data-responsive-toggler class="hidden lg:flex lg:fx-hidden-hidden items-center">
                        @foreach ($sizeTriggers as $trigger)
                            <button data-preview-size-trigger
                                ...
                                class="px-2 py-1 flex text-sm fx-active:bg-muted/60 border border-transparent fx-active:border-border-strong/40 rounded-[5px] ease-linear duration-200 font-300 text-foreground fx-active:text-title-foreground  cursor-pointer">
                                <span aria-hidden="true" class="flex iconify {{ $trigger['icon'] }} text-base" />
                            </button>
                        @endforeach
                        <span class="hidden lg:flex lg:ml-2 lg:mr-1 w-px bg-surface-elevated h-3"
                            aria-hidden="true"></span>
                    </div>
                    <button data-to-fullscreen data-target="mob"
                        class="px-2 py-1 flex fx-hidden:hidden gap-2 items-center text-sm fx-active:bg-background rounded-[5px] ease-linear duration-200 font-300 text-foreground fx-active:text-title-foreground ">
                        <span aria-hidden="true" class="flex iconify ph--corners-out text-base"></span>
                    </button>
                </div>
                <span class="mx-2.5 hidden sm:flex relative w-px bg-surface-elevated h-6" aria-hidden="true"></span>
                <div class="hidden sm:flex">
                    ...
                    <x-ui.button size="none" data-copy-code variant="none"
                        ...
                        <span class="text-muted-foreground ml-1">php php artisan flexi:addui01</span>
                    </x-ui.button>
                </div>
            </div>
        </x-ui.tabs.list-wrapper>
        <x-ui.tabs.panel-wrapper
            class="bg-background rounded-md h-[calc(100vh-10rem)] max-h-[51rem] overflow-hidden flex border border-gray-50 dark:border-gray-900 w-full relative">
            <x-ui.tabs.panel active id="previewbox" aria-labelledby="previewbox"
                class="w-full group ring-offset-bg bg-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 data-[screen=maximized]:fixed data-[screen=maximized]:border-0 data-[screen=maximized]:h-full data-[screen=maximized]:w-full data-[screen=maximized]:left-0 data-[screen=maximized]:top-0 data-[screen=maximized]:z-120">
