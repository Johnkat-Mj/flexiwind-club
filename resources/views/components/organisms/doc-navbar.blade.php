    ...
    <nav class="docs-container px-4 sm:px-8 flex justify-between h-full items-center">
        <div class="flex items-center gap-3">
            <div class="flex lg:hidden -ml-1.5">
                <button data-sidebar-trigger aria-label="toggle sidebar"
                    class="flex flex-col space-y-2 text-muted-foreground hover:bg-subtle ease-linear duration-200 px-1.5 py-2.5 lg:hidden group pr-2 border-r border-muted mr-2">
        ...
        <div class="sm:max-w-sm flex-1 mr-2 sm:mr-0 flex">
            <x-ui.modal.trigger modal-id="search-modal" size="none" variant="none" radius="none"
                class="px-3 h-9 border border-border-input shadow-sm shadow-muted/20 bg-subtle/60 hover:bg-muted/60 w-full flex justify-between items-center rounded-ui">
                <span class="mr-3 iconify ph--magnifying-glass text-sm text-muted-foreground"></span>
        ...
        <div class="flex items-center gap-px">
            <x-atoms.ui-link href="https://github.com/unoforge/flexiwind" aria-label="Link to github repo"
                class="hidden min-[22rem]:d-flex-place-center size-8 hover:bg-muted rounded-lg text-foreground hover:text-title-foreground ">
                <span class="iconify ph--github-logo"></span>
            </x-atoms.ui-link>
            <button x-on:click="$store.theme.toggle()" aria-label="toggle theme"
                class="flex d-flex-place-center size-8 hover:bg-muted rounded-lg text-foreground hover:text-title-foreground  relative">
                <span
