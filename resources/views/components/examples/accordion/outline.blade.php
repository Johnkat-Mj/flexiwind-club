...
<x-ui.accordion class="space-y-2">
    @foreach ($items as $item)
        <x-ui.accordion.item :id="'item-' . $item['id']" variant="outline" class="rounded-md">
            <x-ui.accordion.trigger class="px-4 py-2 text-subtitle font-medium text-lg">
                {{ $item['title'] }}
                <x-ui.accordion.indicator/>
            </x-ui.accordion.trigger>
            <x-ui.accordion.content>
                <p class="p-4 text-foreground">
                    {{ $item['description'] }}
                </p>
            </x-ui.accordion.content>
        </x-ui.accordion.item>
