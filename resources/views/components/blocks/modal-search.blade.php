    ...
    <x-ui.modal.content x-data="searchDocs" :closable="false" no-gutter size="md"
        ...
        <x-ui.modal.header padding-none class="relative w-full">
            <x-ui.input @input="search" data-input-search variant="unstyled"
                class="w-full ps-9 text-foreground focus:outline-none border-0 ui-form-input-lg" placeholder="Search components, docs..."
                x-model="query" />
            <span aria-hidden="true"
                class="iconify ph--magnifying-glass absolute left-3.5 top-3 text-muted-foreground text-sm"></span>
        </x-ui.modal.header>
        <x-ui.modal.body data-search-results
            class="[--gutter:--spacing(0)] overflow-y-auto bg-surface border border-border ui-card [--card-radius:var(--radius-ui)] [--card-padding:--spacing(0.5)] shadow-sm">
            <div x-show="results.length" class="px-1 space-y-0.5">
                <template x-for="item in results" :key="item.slug">
                    <a :href="item.slug"
                        class="flex w-full items-center gap-4 px-3 py-2 inner-radius hover:bg-muted/40 relative before:absolute before:left-10 before:right-0 before:bottom-0 before:flex before:h-0.5 before:bg-linear-to-l before:from-muted/30 before:via-muted/90 before:to-muted/30">
                        <div class="flex min-w-max text-muted-foreground">
                        ...
                        <div class="flex-1 overflow-hidden flex flex-col">
                            <div class="text-sm font-medium text-title-foreground  line-clamp-1" x-text="item.title"></div>
                            <div class="text-xs text-muted-foreground line-clamp-1" x-text="item.description"></div>
                        </div>
                    </a>
                </template>
            </div>
                ...
                <div class="size-full d-flex-place-center flex-col text-center py-4">
                    <svg width="128" height="128" viewBox="0 0 128 128" fill="none"
                        ...
                        <g clip-path="url(#clip0_33_172)">
                            <circle cx="60.7768" cy="62.7768" r="60.7768" fill="currentColor" class="text-muted/50 dark:text-border" />
                            <circle cx="60.7769" cy="62.7769" r="47.2709" fill="#E4ECFE" class="fill-muted" />
                            <path
                                ...
                                class="text-muted/70 dark:text-border stroke-primary-400 dark:stroke-primary-900" stroke-width="2" />
                    ...
                    <span class="text-sm font-medium text-title-foreground  mt-3">
                        No result found
                    </span>
        ...
        <x-ui.modal.footer class="[--gutter:--spacing(2)] flex items-center justify-between text-xs text-muted-foreground">
