<div class="flex flex-col w-full space-y-6">
    <x-ui.breadcrumbs class=" text-muted-foreground">
        <x-ui.breadcrumbs.item href="#"
            class="ease-linear hover:text-subtitle px-1.5 py-px border border-border rounded-full">
            Home
        </x-ui.breadcrumbs.item>
        <x-ui.breadcrumbs.item href="#"
            class="ease-linear hover:text-subtitle px-1.5 py-px border border-border rounded-full">
            Navigation
        </x-ui.breadcrumbs.item>
        <x-ui.breadcrumbs.item
            active active-class="px-1.5 py-px rounded-full border border-border-strong bg-subtle text-title-foreground  font-semibold">
            Current Page
        </x-ui.breadcrumbs.item>
    </x-ui.breadcrumbs>
    <x-ui.breadcrumbs class="text-muted-foreground">
        <x-ui.breadcrumbs.item href="#"
            class="ease-linear hover:text-subtitle px-1.5 py-px border border-border rounded-full">
            Home
        </x-ui.breadcrumbs.item>
        <x-ui.breadcrumbs.item href="#"
            class="ease-linear hover:text-subtitle px-1.5 py-px border border-border rounded-full">
            Navigation
        </x-ui.breadcrumbs.item>
        <x-ui.breadcrumbs.item
            active active-class="px-1.5 py-px rounded-full border text-white bg-primary border-primary font-semibold">
            Current Page
        </x-ui.breadcrumbs.item>
    </x-ui.breadcrumbs>
    <x-ui.breadcrumbs class="text-muted-foreground">
        <x-ui.breadcrumbs.item href="#"
            class="ease-linear hover:text-subtitle gap-x-1.5 px-1.5 py-px rounded-full bg-subtle hover:bg-muted">
            <span aria-hidden="true" class="flex iconify ph--house size-4 mr-1"></span>
            Home
        </x-ui.breadcrumbs.item>
        <x-ui.breadcrumbs.item href="#"
            class="ease-linear hover:text-subtitle gap-x-1.5 px-1.5 py-px rounded-full bg-subtle hover:bg-muted">
            <span aria-hidden="true" class="flex iconify ph--link-break size-4 mr-1"></span>
            Navigation
        </x-ui.breadcrumbs.item>
        <x-ui.breadcrumbs.item active
            active-class="gap-x-1.5 text-bg font-semibold px-1.5 py-px rounded-full bg-title-foreground">
            <span aria-hidden="true" class="flex iconify ph--link-simple-horizontal size-4"></span>
            Current Page
        </x-ui.breadcrumbs.item>
    </x-ui.breadcrumbs>

</div>
